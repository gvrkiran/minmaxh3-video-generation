 ...
 *
 * Identical to the garden's and the painting's routes: every kind of reel is the same folder
 * shape under `user/`, and `reels.py` renders any of them from its spec. Kept as its own route
 * so the pages never share a URL that one of them might later need to change.
 */
