/**
 * Start a list of painting ideas, or read where it has got to.
 *
 ...
 *
 * The driver and the lock are shared with the gardening lists: one graphics card, one list at
 * a time, whichever kind it is. GET filters to painting lists only, so this page never shows
 * a gardening list as its own.
 */
...
export async function POST(request: Request) {
  try {
    ...
    const { id, skipped } = await createBatch(body.ideas, "paint");
...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ batches: await listBatches("paint") });
  } catch (caught) {
