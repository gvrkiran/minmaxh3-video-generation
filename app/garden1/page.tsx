...
/**
 * Tota, list mode -- paste a list of ideas, walk away, come back to videos.
 *
 * The sibling of /garden, with the approval screen deliberately taken out. That screen is
 * right for the case it was built for: one tip, typed by someone who is sitting there, where
 * showing three lines of narration first turns a twenty-minute mistake into a five-second
 * one. It is wrong for this case, where a list is written in advance by the person who will
 * judge the results afterwards, and the whole point is not to have to be present.
 *
 * So this page is written for a different reader. /garden opens in Telugu, hides every
 * number, and describes waiting in minutes. This one is an operator's console: English, one
 * row per idea, the real error text on a failure, and the bill stated before the button is
 * pressed. The ideas themselves can still be typed in Telugu or in Telugu spelled with
 * English letters -- that part is unchanged, because it is the model that has to read them.
 *
 ...
 * the same state -- which is the only way "start it and walk away" can actually be true.
 */
...
const STATUS_STYLE: Record<ItemStatus, React.CSSProperties> = {
  ...
  rendering: { background: "#e2efe4", color: "#2f5d3a" },
  done: { background: "#2f5d3a", color: "#fff" },
...
export default function GardenBatch() {
  ...
  const loadList = useCallback(async () => {
    try {
      const got = await fetch("/api/garden1/batch", { cache: "no-store" });
   ...
   *
   * No immediate first tick. Every route into this state -- opening the page, starting a
   * list, resuming one -- has just been handed a fresh batch by the response that got it
   * here, so firing one straight away would only repeat a request that has just returned.
   */
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      try {
        const got = await fetch(`/api/garden1/batch?id=${encodeURIComponent(id)}`,
          { cache: "no-store" });
  ...
  async function start() {
    ...
    try {
      const got = await fetch("/api/garden1/batch", {
        method: "POST",
  ...
  async function control(action: "stop" | "resume") {
    ...
    try {
      const got = await fetch("/api/garden1/control", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  /** Retry every unfinished item across every list, gathered into one new list. */
        ...
        {item.status === "done" && (
          open ? (
            <video
              ...
              autoPlay
              playsInline
              src={`/api/garden/file?slug=${encodeURIComponent(item.slug)}&rel=${
  ...
  return (
    <main style={S.page}>
      <header style={S.header}>
        <div>
          <div style={S.brand}>Tota &mdash; list</div>
          <div style={S.tagline}>
            Paste your ideas, one per line. They are made one after another, no approving.
          </div>
        </div>
        <a href="/garden" style={S.link}>one at a time &rarr;</a>
      </header>
      ...
      {/* Always rendered, never hidden behind a running list. Hiding it meant that opening
          ...
          as a broken page rather than as a busy one. While a list runs it stays here to be
          typed into, and only the button waits. */}
      <section style={{ ...S.card, order: batch?.live ? 2 : 1 }}>
        <h1 style={S.h1}>Your ideas</h1>
        <p style={S.help}>
          One idea per line. Telugu, or Telugu spelled with English letters &mdash; either is
          fine. Blank lines and lines starting with # are ignored. Up to {MAX_IDEAS} at a
        ...
        <textarea
          ...
          placeholder={"once a month dig around the plant and pour jaggery water\n"
            + "veepa nune vaaraaniki oka sari kottali\n"
            + "biyyam kadigina neellu mokkalaki poyali"}
          rows={9}
          spellCheck={false}
        />
        <div style={S.tools}>
        ...
        <button
          style={{ ...S.primary,
            ...(busy || count === 0 || count > MAX_IDEAS || batch?.live ? S.disabled : {}) }}
          disabled={busy || count === 0 || count > MAX_IDEAS || Boolean(batch?.live)}
          onClick={() => void start()}
        >
          {busy ? "Starting..."
...
/* Inline styles, matching /garden and /story -- this app ships no CSS framework, and a
   stylesheet for one page would be a second place to look. Denser than /garden on purpose:
   this page shows twenty-five rows at once and is read at a desk, not outdoors. */
const S: Record<string, React.CSSProperties> = {
  ...
  brand: { fontSize: 28, fontWeight: 700, color: "#2f5d3a", lineHeight: 1.2 },
  tagline: { fontSize: 15, color: "#6b6257", marginTop: 4, maxWidth: 460 },
  link: { fontSize: 14, color: "#57503f", flexShrink: 0, paddingTop: 6 },
  card: {
  ...
  upload: {
    fontSize: 14, color: "#2f5d3a", cursor: "pointer",
  ...
  resume: {
    background: "none", border: "1.5px solid #a9c1ae", borderRadius: 999,
    padding: "7px 16px", fontSize: 14, color: "#2f5d3a", cursor: "pointer",
    fontFamily: "inherit", flexShrink: 0,
  },
  ...
  rowStage: { fontSize: 13.5, color: "#2f5d3a", marginTop: 7 },
  rowError: {
  ...
  track: {
    height: 7, background: "#e8e3d8", borderRadius: 999, overflow: "hidden",
  ...
  watch: {
    ...
    borderRadius: 999, padding: "6px 15px", fontSize: 13.5, color: "#2f5d3a",
    cursor: "pointer", fontFamily: "inherit",
  },
  video: {
